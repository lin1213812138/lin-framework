...
export class UserController {
  ...
  async create(@Body() dto: CreateUserDto): Promise<ApiResponse<{ _id: string }>> {
    ...
    return { code: 0, message: 'user created', data: { _id }, timestamp: Date.now(), requestId: '' };
  }
}
```
...
```typescript
// api/user.ts
export function fetchUsers(query: QueryUserDto): Promise<ApiResponse<PaginatedResult<UserResponseDto>>> {
...
describe('UserService', () => {
  ...
  describe('findById', () => {
    ...
    it('should throw when user not found', async () => {
      ...
      await expect(
        service.findById('b2c3d4e5-f6a7-8901-bcde-f12345678901'),
